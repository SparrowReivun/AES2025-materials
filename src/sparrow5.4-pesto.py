...
def PYIN(audio, sr):
    ...
    print("Using PYIN for pitch estimation...")
...
def PESTO(audio_path):
    ...
    print("Using PESTO for pitch estimation...")
...
def process_audio_files_in_path(path):
    ...
    output_father_dir = os.path.join("output", path.split("/")[-1])
    output_dir = os.path.join(output_father_dir, "p_lufs")
    output_lufs_dir = os.path.join(output_father_dir, "lufs")
    output_centroid_dir = os.path.join(output_father_dir, "centroid")
    ...
    save_as_json(results, output_father_dir, 'results.json')
    save_as_csv(results, output_father_dir, 'results.csv')

    # Bounce mixed audio to stereo for each directory
    bounce_to_stereo(output_father_dir, 'p_lufs', '_equal_p_loudness')
    bounce_to_stereo(output_father_dir, 'lufs', '_equal_loudness')
    bounce_to_stereo(output_father_dir, 'centroid', '_centroid_adjustment')


if __name__ == '__main__':
    total_path = 'data' # 这里假设是总路径
